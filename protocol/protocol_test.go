...
func TestCreateStatusFromResponse(t *testing.T) {
...
func TestFailOnInvalidStatusResponse(t *testing.T) {
    // This should invalid input but it looks a lot like valid input
    input := []byte("")

    _, err := newStatus(input)

    if err == nil {
        t.Error("Expected an error to be thrown at illegal input.")
    }
}
